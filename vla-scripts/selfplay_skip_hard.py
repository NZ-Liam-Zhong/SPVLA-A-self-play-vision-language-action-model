...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            ##这里打印loss的大小和size
            print("shape of normalized loss",normalized_loss.shape)
            ##loss_threshold表示太难就不学了
            loss_threshold=0.5
            valid_mask = normalized_loss < loss_threshold
            ...
            filtered_losses = torch.where(valid_mask, losses, torch.tensor(0.0, device=device_id))
            print("filtered_losses",filtered_losses)

            print("cfg.grad_accumulation_steps",cfg.grad_accumulation_steps)

            valid_loss_count = valid_mask.sum().item()

            print("valid_loss_count",valid_loss_count)

            losses2=filtered_losses/ valid_loss_count
